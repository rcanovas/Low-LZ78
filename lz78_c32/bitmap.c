...
void saveBitmap(FILE *f, bitmap B)
 {
    uint ns, nb, indicator, aux;
    
    ns = (B->n+256-1)/256; nb = 256/W;
    
    if (fwrite(B->sdata, sizeof(uint), ns, f) != ns) {
       fprintf(stderr,"Error: Cannot write Bitmap on file\n");
       exit(1);
    }
    if (fwrite(B->bdata, sizeof(byte), ns*nb, f) != ns*nb) {
       fprintf(stderr,"Error: Cannot write Bitmap on file\n");
       exit(1);
    }
    if (B->sdata_0 != NULL) {
       indicator = 1;
       if (fwrite(&indicator, sizeof(uint), 1, f) != 1) {
          fprintf(stderr,"Error: Cannot write Bitmap on file\n");
          exit(1);
       }
       if (fwrite(B->sdata_0, sizeof(uint), ns, f) != ns) {
          fprintf(stderr,"Error: Cannot write Bitmap on file\n");
          exit(1);
       }
       if (fwrite(B->bdata_0, sizeof(byte), ns*nb, f) != ns*nb) {
          fprintf(stderr,"Error: Cannot write Bitmap on file\n");
          exit(1);
       }
    }
...
bitmap loadBitmap(FILE *f, uint n, uint *data)
 {
    ...
    B->n = n;
    B->data = data;
    ns = (B->n+256-1)/256; nb = 256/W; // adjustments
    B->bdata = malloc (ns*nb*sizeof(byte));
    B->sdata = malloc (ns*sizeof(uint));
    
    if (fread(B->sdata, sizeof(uint), ns, f) != ns) {
       fprintf(stderr,"Error: Cannot read Bitmap from file\n");
       exit(1);
    }
    
    if (fread(B->bdata, sizeof(byte), ns*nb, f) != ns*nb) {
       fprintf(stderr,"Error: Cannot read Bitmap from file\n");
       exit(1);
    }

    if (fread(&indicator, sizeof(uint), 1, f) != 1) {
       fprintf(stderr,"Error: Cannot read Bitmap from file\n");
       exit(1);
    }
    if (indicator) {
       B->bdata_0 = malloc (ns*nb*sizeof(byte));
       B->sdata_0 = malloc (ns*sizeof(uint));
       if (fread(B->sdata_0, sizeof(uint), ns, f) != ns) {
          fprintf(stderr,"Error: Cannot read Bitmap from file\n");
          exit(1);
       }
       if (fread(B->bdata_0, sizeof(byte), ns*nb, f) != ns*nb) {
          fprintf(stderr,"Error: Cannot read Bitmap from file\n");
          exit(1);
       }
    }
...
void destroyBitmap(bitmap B)
 { 
    ...
    free(B->bdata);
    free(B->sdata);
    if (B->bdata_0) free(B->bdata_0);
    if (B->sdata_0) free(B->sdata_0);
    free(B);
 }
