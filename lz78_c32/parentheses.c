...
parentheses createParentheses (uint *string, uint n, bool bwd)
   ...
   { parentheses P;
     ...
     P->data = string;
     P->n = n;
     P->bdata = createBitmap (string,n,true);
     nbits = bits(n-1);
     s = nbits*W;
     P->sbits = bits(s-1);
...
uint findclose (parentheses P, uint i)
   ...
   { uint bitW;
	...
	{ if (!minres || (res < minres)) 
	     if (excess(P,i+res+1) == myexcess) minres = res;
	...
	// finally, it has to be a far pointer
     ...
     while (res)
	{ if (!minres || (res < minres)) 
	     if ((i+res+1 < P->n) && (excess(P,i+res+1) == myexcess)) minres = res;
...
uint findparent (parentheses P, uint i)
   ...
   { uint bitW;
	...
	// ok, it's not a small distance, try with hashing btable
     ...
     while (res)
	{ if (!minres || (res < minres)) 
	     if (excess(P,i-res) == myexcess) minres = res;
